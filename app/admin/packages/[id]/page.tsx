...
async function resolveNumericIdFromUuid(uuidOrId: string): Promise<string> {
    ...
    const listRes = await laravelServerFetch("/backend/admin/packages?status=active");
...
export default async function AdminPackageDetailsPage({
    ...
    // 1) package details
    const pRes = await laravelServerFetch(`/backend/admin/packages/${id}`);
    ...
    const cRes = await laravelServerFetch(`/backend/admin/packages/${id}/cards${cQuery}`);
